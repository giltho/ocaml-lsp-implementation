(* useful for later *)
(* type onlyDynamicRegistration = { dynamicRegistration: (bool [@default
   false]); }[@@deriving yojson { strict = false }] *)
...
(* type referencesCapabilities = onlyDynamicRegistration let
   referencesCapabilities_of_yojson = onlyDynamicRegistration_of_yojson let
   referencesCapabilities_to_yojson = onlyDynamicRegistration_to_yojson *)
