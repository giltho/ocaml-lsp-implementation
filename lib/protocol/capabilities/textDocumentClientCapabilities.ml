...
let defaultSynchronisation = Result.get (synchronisationCapabilities_of_yojson (`Assoc []))
...
let defaultCompletionItemCapabilities = Result.get (completionItemCapabilities_of_yojson (`Assoc []))
...
let defaultCompletionCapabilities = Result.get (completionCapabilities_of_yojson (`Assoc []))
...
let defaultHoverCapabilities = Result.get (hoverCapabilities_of_yojson (`Assoc []))
...
type signatureHelpCapabilities = {
  dynamicRegistration: (bool [@default false]);
  signatureInformation: (signatureInformationCapabilities [@default defaultSignatureInformationCapabilities]);
}[@@deriving yojson]

let defaultSignatureHelpCapabilities = Result.get (signatureHelpCapabilities_of_yojson (`Assoc []))
...
type t = {
  ...
  completion:      (completionCapabilities [@default defaultCompletionCapabilities]);
